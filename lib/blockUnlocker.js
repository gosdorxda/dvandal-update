            ...
            blocks.forEach(function(block){
                ...
                if (block.workerScores) {
                    ...
                    Object.keys(block.workerScores).forEach(function (worker) {
                        ...
                        log('info', logSystem, 'added pending to %s for %d shares: %d', [worker, block.workerScores[worker], workerReward]);

                    unlockedBlocksCommands.push(['hincrby', config.coin + ':workers:' + worker, 'balance', workerReward]);
                    unlockedBlocksCommands.push(['zadd', config.coin + ':blockstat:' + worker, block.height,
                    [
                      block.time,
                      now,
                      block.reward,
